...
if [[ "$1" == *"qdb-benchmark"* ]]; then
    BENCHMARK_TARBALL=$1; shift
fi
if [[ "$1" == *"qdb-railgun"* ]]; then
    RAILGUN_TARBALL=$1; shift
fi

PACKAGE_NAME=$(basename "$SPEC_FILE" '.spec')
PACKAGE_VERSION=$(echo "$PACKAGE_TARBALL" | sed -r 's/.*qdb-(.*)-linux-.*/\1/')

export PACKAGE_VERSION PACKAGE_NAME PACKAGE_TARBALL BENCHMARK_TARBALL RAILGUN_TARBALL
